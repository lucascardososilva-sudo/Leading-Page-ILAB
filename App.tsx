...
// Critical LCP Component - Loaded Synchronously
...
const App: React.FC = () => {
  ...
  // Noise Overlay (Moved from CSS body::before to Hardware Accelerated Div)
  const NoiseOverlay = () => (
  ...
  return (
    <SmoothScrollLayout>
      <div className="min-h-screen font-sans selection:bg-fox-200 selection:text-navy-900 relative">
        <NoiseOverlay />

        {/* Scroll Progress Bar */}
        <motion.div
          className="fixed top-0 left-0 right-0 h-1 bg-fox-500 origin-left z-50 pointer-events-none"
          style={{ scaleX }}
        />
        ...
        <main>
          {/* Hero is Critical - No Suspense */}
          <Hero />

          {/* BELOW THE FOLD - Lazy Loaded & Error Protected */}
          <ErrorBoundary fallback={<div className="py-20 text-center">Carregando conteúdo...</div>}>
            <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-cream"><div className="w-8 h-8 border-4 border-fox-500 border-t-transparent rounded-full animate-spin"></div></div>}>
